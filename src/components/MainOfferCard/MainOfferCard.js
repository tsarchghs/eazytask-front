...
const MainOfferCard = props => {
    return (
        <div style={{ cursor: "pointer"}} onClick={() => this.props.history.push(`/task/${props.task.id}/edit/offers/${props.id}`)} className="home__card" style={{ backgroundImage: `url("${props.task.thumbnail || window.__THUMBNAIL_DEFAULT_PICTURE__}")` }}>
